...
var Parser5 = /*@__PURE__*/ getDefaultExportFromCjs(parser);
 ...
 */ function all$1(state, nodes) {
    ...
    while(++index < nodes.length){
        ...
        result[index] = one$1(state, nodes[index]);
    }
    return result;
}
/**
...
/** @type {import('unified').Plugin<[Options?] | Array<void>, string, Root>} */ function rehypeParse(options) {
     ...
     */ function onerror(error) {
            ...
            if (level) {
                /* c8 ignore next */ const info = errors[name] || {
                    reason: '',
                    description: '',
                    url: ''
                };
...
function toFragment(element, { trees, lang, title, caption, inline = false, keepBackground = true, lineNumbersMaxDigits = 1, onVisitTitle, onVisitCaption }) {
...
function rehypePrettyCode(options = {}) {
    const { grid = true, theme = 'github-dark-dimmed', keepBackground = true, tokensMap = {}, filterMetaString = (v)=>v, getHighlighter: getHighlighter$1 = getHighlighter, onVisitLine, onVisitHighlightedLine, onVisitHighlightedChars, onVisitTitle, onVisitCaption } = options;
